...
describe("Resource API", () => {
    ...
    describe('GET /view-resources', ()=> {
        it("should return all resources", (done) => {
            chai.request(baseUrl).get('/view-resources').end((err,res) =>{
    ...
    describe('POST /add-resource', () => {
        it("should return 500 for validation errors", (done) => {
            chai.request(baseUrl).post("/add-resource")
            .send({name:  "Test Resource", location: "Test Location", description: "Short", owner: "invalid-email"})
            .end((err,res) => {
        ...
        it("should add a new resource", (done) => {
            chai.request(baseUrl).post("/add-resource")
            .send({name: "Test Resource", location: "Test Location", description: "Test Description", owner: "test@gmail.com"})
            .end((err,res) => {
                expect(res).to.have.status(201)
                expect(res.body).to.be.an("array")
                expect(res.body.length).to.equal(count + 1)
                resourceId = res.body[res.body.length - 1].id //store the id of newly added resource
                done()
            })
        })    
    })
    ...
    describe('PUT /edit-resource/:id', () => {
        it('should update an existing resource', (done) => {
            chai.request(baseUrl).put(`/edit-resource/${resourceId}`)
    ...
    describe('DELETE /delete-resource/:id', () => {
        it('should delete an existing resource', (done) => {
            chai.request(baseUrl).delete(`/delete-resource/${resourceId}`).end((err, res) => {
                expect(res).to.have.status(201);
